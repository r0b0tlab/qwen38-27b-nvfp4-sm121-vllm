#!/usr/bin/env bash
# MTP K-ladder for Qwen3.8-27B NVFP4 on node2.
# Per K: relaunch serve (MTP checkpoint), wait ready, verify spec-decode
# markers, semantic gate, acceptance counters, quick dedicated-c1.
# Usage: KLIST="2 3" bash run_mtp_ladder.sh <image> [run-tag]
set -uo pipefail
...
for K in $KLIST; do
  ...
  ssh -o BatchMode=yes "$NODE2" "docker rm -f qwen38-27b >/dev/null 2>&1; \
    docker run -d --name qwen38-27b --runtime nvidia -e NVIDIA_VISIBLE_DEVICES=0 \
    ...
    --kv-cache-dtype auto --trust-remote-code --enforce-eager \
...
for rep in range(3):
    ...
    rows.append({"rep": rep, "completion_tokens": ct, "elapsed_s": round(el, 2),
                 "tok_s": round(ct / el, 2)})
out = {"rows": rows,
       "median_tok_s": sorted(r["tok_s"] for r in rows)[1]}
