...
curl -s -m 5 http://192.168.0.2:8000/health >/dev/null 2>&1 || { echo READY_TIMEOUT; exit 4; }
...
timeout 600 python3 "$ROOT/repo/scripts/run_semantic_gate.py" --base-url http://192.168.0.2:8000 --output "$EV/semantic-gate.json" >/dev/null 2>&1
...
cd "$ROOT" && python3 repo/scripts/run_quality_set.py --base-url http://192.168.0.2:8000 --run-id final-sota --set artifacts/quality-200.jsonl 2>&1 | tail -3
