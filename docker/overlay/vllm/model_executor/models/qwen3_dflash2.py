...
class CandidateSelector(nn.Module):
    ...
    def forward(
        ...
        # SM121: profile dummy [N,H]@ [256,H] BF16 through ReplicatedLinear /
        # BasevLLMParameter hits CUBLAS_STATUS_INTERNAL_ERROR. Project in
        # FP32 via .data in small chunks (same math, no cublas-ex BF16 path).
        orig = hidden_states.shape
        flat = hidden_states.reshape(-1, orig[-1]).contiguous()
        if flat.numel() == 0:
            return unary_logits.new_zeros(
                orig[0], orig[1], self.top_k, self.top_k
            )
        weight = self.hidden_projection.weight
...
class DFlash2Qwen3ForCausalLM(DFlashQwen3ForCausalLM):
    ...
    def compute_candidates(
        ...
        # r0b0tlab: stock PR #52816 rejected any non-UnquantizedEmbeddingMethod
        # head. Our Qwen3.8-27B NVFP4 target lm_head is W4A16_NVFP4. The apply
        # kernel is the same path LogitsProcessor uses; verify stays lossless
        # through the target's real logits. tp>1 is untested here.
        if get_tensor_model_parallel_world_size() > 1 and not isinstance(
            self.lm_head.quant_method, UnquantizedEmbeddingMethod
        ):
            raise ValueError(
                "DFlash2 selector with a quantized target lm_head is only "
                "supported at tp=1."
            )
        apply = getattr(getattr(self.lm_head, "quant_method", None), "apply", None)
        if apply is None:
            raise ValueError(
                "DFlash2 selector needs lm_head.quant_method.apply "
                f"(got {type(getattr(self.lm_head, 'quant_method', None))})."
            )

        selector = self.model.candidate_selector
        logits = apply(self.lm_head, hidden_states, bias=None)
        ...
        # tp=1 target embed does not mask; flashinfer topk can emit >= dim
        # on NaN rows. Keep candidates inside the codebook / embed table.
        ids.clamp_(0, logits.shape[-1] - 1)
